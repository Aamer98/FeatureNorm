...
def shift_affine(model, source_stat, device):
    total_shift = 0
    i = 0
    for layer in model.modules():
        if isinstance(layer, nn.BatchNorm2d):
            ...
            shift_mean = (source_mean - target_mean)
            shift_var = (source_var - target_var)
            total_shift += torch.sum(shift_mean)

            # shift affine
            rand_mean_shift = (torch.rand(len(source_mean)).to(
                device) * shift_mean.to(device)).to(device)
            rand_var_shift = (torch.rand(len(source_var)).to(
                device) * shift_var.to(device)).to(device)

            layer.bias = nn.Parameter(
                layer.bias + (rand_mean_shift * layer.weight / (source_var - rand_var_shift))).to(device)
            layer.weight = nn.Parameter(
                layer.weight * (1/(1-(rand_var_shift/source_var)))).to(device)
            i += 1
    return total_shift
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='STARTUP')
    parser.add_argument('--dir', type=str, default='./logs/BAS_in/EuroSAT',
